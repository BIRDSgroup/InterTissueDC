...
readBMfile <- function(BR){
  file <- data.frame(read_tsv(paste("data/AMP-AD_MSBB_MSSM_", BR, ".normalized.sex_race_age_RIN_PMI_exonicRate_rRnaRate_batch_adj.tsv", sep="")))
  row.names(file) <- file[,1]
  return(as.data.frame(t(file[,-1])))
}

# ENSEMBL to HGNC mapping -------------------------------------------------
ensid_sampleid <- read.csv(paste("data/msbb_metadata_mapping_and_group_assignment.csv"), stringsAsFactors = FALSE)

find_genes_mapped <- function(){
  ens_ids <- ensid_sampleid[,1]
  
  gencode_mapping <- read.csv(paste("data/Mapped_h37_ens_to_gene.csv", sep = ""), 
...
rna_metadata_processed <- read.csv(paste("data/rna_metadata_onlyBAM_noExclude_noDup_Nov2018.csv",
...
clinical <- read.csv(paste("data/MSBB_clinical.csv", sep = ""),
                     stringsAsFactors = FALSE)
...
my_getAllSPVs <-
  function(brainRegion = "", data, grp, dataTag, method=c("mixed", "raw", "residual", "SVA"), plot=F, mix.par=0.3){
    ...
    if(brainRegion != ""){write.csv(Us, paste("results/", brainRegion, "U_SVD_CellCODE.csv", sep = ""))}
