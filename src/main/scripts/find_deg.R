...
if(ct_corrected == TRUE){
  ad_ctl_gExp <- read.csv(paste(path_results, BR, "_AD_CTL_cellTypeCorrected.csv", sep = ""),
                          stringsAsFactors = FALSE, row.names = 1)
  colnames(ad_ctl_gExp) <- ensid_sampleid$ENS_ID
} else{
  bm <- read.csv(paste(path_geneExp,BR,'.csv',sep=""),stringsAsFactors = FALSE, row.names = 1)
...
mapping_file <- read.csv(paste(path_data, "Mapped_h37_ens_to_gene.csv", sep = ""),
                         stringsAsFactors = FALSE)
deg$gene <- mapping_file$gene_name[match(row.names(deg), mapping_file$gene_id)]

write.csv(deg, paste(path_results, BR, ct_file_name, "_DEG_WilcoxonRankSumTest.csv", sep = ""))
